...
import { toggleSidebar, toggleCart } from "../../features/Slice";
import productContext from "../../Context/productContext";

const Navbar = () => {
  const dispatch = useDispatch();
  const cart = useSelector((state) => state.cart);
  const { isSidebarOpen, isCartOpen } = useSelector((state) => state);
  ...
  //   const filteredData = usersData.filter((user) => user.uid === userUid);
  //   console.log(filteredData[0].products);
  // };
  return (
    <>
      <div
        ref={navRef}
        className="sticky top-0 z-20 flex items-start justify-between p-6 "
      >
        <div className="h-[83px] overflow-hidden">
          <Link to={"/"}>
            <div
              ...
              } duration-200 flex flex-col gap-5`}
            >
              <svg
                width="106"
                height="83"
              ...
              >
                <g clipPath="url(#clip0_2228_162)">
                  <path
                    ...
                    fill={`${isSidebarOpen || isCartOpen ? "white" : "black"}`}
                  ></path>
                  <path
        ...
        <div
          className={`flex items-center ${
            isSidebarOpen || isCartOpen ? "text-white" : "text-black"
          }`}
        >
          <div
            className={`${
              scrolled ? "hidden" : "flex gap-10 p-2 text-xs"
            } duration-300`}
          >
            {user && Object.keys(user).length !== 0 ? (
              <div className="flex flex-col w-full text-center cursor-pointer lg:text-left md:text-center shoptosupport lg:w-max">
                <p className="shop-text mb-1 uppercase text-[12px]">
            ...
            <div className="flex flex-col w-full text-center cursor-pointer lg:text-left md:text-center shoptosupport lg:w-max">
              <Link to={"/shop"}>
                <p className="shop-text mb-1 text-[12px]">SHOP</p>
              </Link>
              <div className="underline-div"></div>
            </div>
            <div className="flex items-start mx-8 ">
              <div className="flex flex-col w-full text-center cursor-pointer lg:text-left md:text-center shoptosupport lg:w-max">
                <p className="shop-text mb-1 text-[12px]">CATERING</p>
                <div className="underline-div"></div>
              </div>
            ...
            <div className="flex flex-col w-full text-center cursor-pointer lg:text-left md:text-center shoptosupport lg:w-max">
              <p className="shop-text mb-1 text-[12px]">DONATE</p>
              <div className="underline-div"></div>
            </div>
          </div>
          <div
            className={`flex p-2 px-3 mb-1 ml-10 ${
              isSidebarOpen || isCartOpen ? "bg-black" : "bg-white"
            } rounded-2xl`}
          >
            <div
              className="mr-8 cursor-pointer hover:scale-75"
            ...
            <div className="relative flex items-start">
              <div
                className="ml-8 duration-300 cursor-pointer hover:-rotate-12"
                onClick={toggleCartHandler}
              >
                {isCartOpen ? <RxCross1 /> : <FiShoppingCart />}
              </div>
              {cart.length === 0 ? (
                ""
              ) : (
                <div className="absolute right-0 -top-0.5 w-1.5 h-1.5 bg-green-400 rounded-full"></div>
              )}
            </div>
          </div>
        </div>
